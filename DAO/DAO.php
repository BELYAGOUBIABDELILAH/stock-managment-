...
function ajouterProduit($obj) {
    ...
    $stmt->execute(array(
        ...
        function updateProduit($obj) {
            $pdo = $this->getPDO();
            $stmt = $pdo->prepare("UPDATE produit SET libelle=?, prixUnitaire=?, quantiteStock=?, prixAchat=?,  idCategorie=? WHERE reference=?;");
            $stmt->execute(array($obj->get("l"), $obj->get("p"), $obj->get("q"), $obj->get("a"),  $obj->get("c"), $obj->get("r")));
        }
        ...
        function getProduitTotal() {
            $pdo = $this->getPDO();
            $stmt = $pdo->prepare("SELECT count(*) as number FROM produit;");
            $stmt->execute();
...
//COMMANDE////////
        ...
        function deleteCommande($id) {
            $pdo = $this->getPDO();
            $stmt = $pdo->prepare('DELETE FROM lignecmd WHERE numeroCmd = :id');
            $stmt->execute([':id' => $id]);
        
            $stmt = $pdo->prepare('DELETE FROM commande WHERE numeroCmd = :id');
            $stmt->execute([':id' => $id]);
        }
        ...
        public function getNbCommandesProduit($reference) {
        $pdo = $this->getPDO();
        $stmt = $pdo->prepare("SELECT COUNT(*) AS total FROM commande WHERE reference = ?");
...
//CATEGORIE////////
        ...
        function ajouterCategorie($obj){
            ...
            $stmt_check = $pdo->prepare("SELECT COUNT(*) FROM categorie WHERE idCategorie = ?");
            $stmt_check->execute([$obj->get("i")]);
            $count = $stmt_check->fetchColumn();

            if ($count > 0) {
                echo "Cette catégorie existe déjà.";
                return; // Sortir de la fonction si la catégorie existe déjà
            }

            // Ajouter la catégorie si elle n'existe pas déjà
            $stmt_insert = $pdo->prepare("INSERT INTO categorie(idCategorie, nomCategorie) VALUES (?, ?)");
            $stmt_insert->execute([$obj->get("i"), $obj->get("n")]);
